...
def link_prediction(head_id, tail_id, entity_embeddings, relation_embeddings):
    ...
    # 计算头实体和尾实体的余弦相似度
    similarities = cosine_similarity(relation_embeddings + head_emb.unsqueeze(0), tail_emb.unsqueeze(0)).squeeze(0)
    top_5_ids = torch.topk(similarities, k=5).indices.tolist()
    return top_5_ids
...
def entity_prediction(head_id, relation_id, entity_embeddings, relation_embeddings):
    ...
    # 计算头实体和关系的组合向量与所有实体的余弦相似度
    similarities = cosine_similarity(entity_embeddings + head_emb.unsqueeze(0) + relation_emb.unsqueeze(0), entity_embeddings)
    top_5_ids = torch.topk(similarities, k=5).indices.tolist()
    return top_5_ids
...
def generate_predictions(test_data, entity_embeddings, relation_embeddings, id_to_entity, id_to_relation):
    ...
    for head_id, relation_id, tail_id in test_data:
        # Link Prediction
