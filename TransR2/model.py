...
def distanceL2(h, r, t, W_r):
    # Apply the transformation matrix for relation r
    transformed_h = np.dot(W_r, h)
    transformed_t = np.dot(W_r, t)
    return np.sum(np.square(transformed_h + r - transformed_t))

def distanceL1(h, r, t, W_r):
    # Apply the transformation matrix for relation r
    transformed_h = np.dot(W_r, h)
    transformed_t = np.dot(W_r, t)
    return np.sum(np.fabs(transformed_h + r - transformed_t))
...
class TransR:
    ...
    def emb_initialize(self):
        ...
        self.transformation = transformation_dict
    ...
    def update_embeddings(self, Tbatch):
        ...
        copy_transformation = copy.deepcopy(self.transformation)
        ...
        for triple, corrupted_triple in Tbatch:
            ...
            W_r_update = copy_transformation[triple[2]]
            ...
            W_r = self.transformation[triple[2]]

            h_corrupt = self.entity[corrupted_triple[0]]
            t_corrupt = self.entity[corrupted_triple[1]]

            if self.L1:
                dist_correct = distanceL1(h_correct, relation, t_correct, W_r)
                dist_corrupt = distanceL1(h_corrupt, relation, t_corrupt, W_r)
            else:
                dist_correct = distanceL2(h_correct, relation, t_correct, W_r)
                dist_corrupt = distanceL2(h_corrupt, relation, t_corrupt, W_r)
        ...
        for r in copy_transformation.keys():
            copy_transformation[r] /= np.linalg.norm(copy_transformation[r])

        # 达到批量更新的目的
        self.entity = copy_entity
        self.relation = copy_relation
        self.transformation = copy_transformation
...
if __name__ == '__main__':
    ...
    transR = TransR(entity_set, relation_set, triple_list, embedding_dim=500, learning_rate=0.001, margin=1, L1=True)
    transR.emb_initialize()
    transR.train(epochs=100)
