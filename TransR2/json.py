...
# 计算得分（改为计算余弦相似度）
def compute_score_batch(heads, relations, tails, metric='cosine'):
    # 计算嵌入向量的余弦相似度
    if metric == 'cosine':
        # 计算 (heads + relations) 与 tails 之间的余弦相似度
        score = F.cosine_similarity(heads + relations, tails, dim=1)
        return score
    else:
        # 如果不是余弦相似度，返回 L2 距离
...
def link_prediction(entity_embeddings, relation_embeddings, test_data, top_k=5, device="cuda"):
    ...
    for head, tail, true_relation in test_data:
        ...
        scores = compute_score_batch(head_batch, relation_batch, tail_batch, metric='cosine')
...
def entity_prediction(entity_embeddings, relation_embeddings, test_data, top_k=5, device="cuda"):
    ...
    for head, relation, true_tail in test_data:
        ...
        scores = compute_score_batch(head_batch, rel_batch, entity_batch, metric='cosine')
        sorted_indices = torch.argsort(scores, descending=True)[:top_k]
...
if __name__ == '__main__':
    ...
    entity2id_file = "/data_disk/libh/KRL/WN18/entity2id.txt"
    relation2id_file = "/data_disk/libh/KRL/WN18/relation2id.txt"
    entity_embedding_file = "entity_50dim"
    relation_embedding_file = "relation_50dim"
    test_data_file = "/data_disk/libh/KRL/WN18/test1.txt"
